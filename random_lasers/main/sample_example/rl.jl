...
# LMat_sparse(1.0+im)
# LMat_dense(1.0+im)
# maximum(abs.(LMat_num(1.0+im).-LMat_sparse(1.0+im)))
# maximum(abs.(LMat_num(11.0-0.02im).-LMat_sparse(11.0-0.02im)))
# maximum(abs.(LMat_num(11.0-0.002im).-LMat_dense(11.0-0.002im)))

# # Define the ranges for the real and imaginary parts of z
# x_vals = range(9.0, stop=12.0, length=150)  # Grid for the real part of z
# y_vals = range(0.0, stop=0.1, length=150)  # Grid for the imaginary part of z

# # Initialize a variable to store the maximum difference
# max_diff = 0.0
# max_z = 0 + 0im  # The z point that maximizes the difference

# # Loop over the entire grid
# for x in x_vals
#     for y in y_vals
#         z = Complex(x, y)  # Construct the complex point z = x + yi
#         diff = maximum(abs.(LMat_num(z) .- LMat_sparse(z)))  # Calculate the absolute difference
#         if diff > max_diff
#             max_diff = diff
#             max_z = z  # Store the point that yields the maximum
#         end
#     end
# end

# # Output the maximum difference and the corresponding point
# println("Maximum difference: $max_diff found at z = $max_z")


# # determinant
# function detLMat_sparse(k)
#     Mat = LMat_sparse(k)
#     return det(Mat)
# end

# # determinant
# function detLMat_dense(k)
#     Mat = LMat_dense(k)
#     return det(Mat)
# end
...
# # Call the function with a specific value of k
# k_value = 2.5+3im
# detLMat_sparse(k_value)
# detLMat_dense(k_value)
# detLMat_num(k_value)
# detLMat_sparse(k_value)
# cos(k_value)
# @timed detLMat_num(k_value)
# @timed detLMat_dense(k_value)
# @timed detLMat_sparse(k_value)
# @timed cos(k_value)
##### END SAVED CHECK
##########################################
...
# CHECK
# detLMat(9.7-0.00000000001im)
# detLMat(9.7)
# detLMat(9.7+r_mesh/1000)
# detLMat(9.7+r_mesh)
# detLMat(9.7+im*r_mesh*10000)
# r_mesh
...
# # Newton-Raphson method with numerical derivative and Im(k)<0 (test version)
# function Newton_Raphson_mod_check(f, initialGuess, real_min, real_max, r_mesh; tol=1e-6, maxIter=10^5, exp_prec=1, r_mesh_fac_tol=500)
...
#         return x, iter
#     else
#         error("Newton_Raphson: maximum number of iterations reached without convergence. Try increase maxIterations")  
#     end
# end
...
# function WS_Newton_Raphson_mod(f, initialGuess, real_min, real_max, r_mesh; tol=1e-10, maxIter=10^5, exp_prec=1, r_mesh_fac_tol=1000)
    
#     x = initialGuess
#     fx = f(x)
#     lamdax = minimum(abs.(eigvals(LMat(x))))
#     iter=0
#     while lamdax > tol && iter < maxIter
#         dfx = num_auto_diff(f, x; h=r_mesh*10.0^(-3))
#         iter += 1
...
# Test to compile 3 min
@timed Newton_Raphson_mod(detLMat, 9.6-0.00000000001im, Re_k_min, Re_k_max, r_mesh; tol=10^90, maxIter=1)
...
##### SAVED DATA ####
# ~ 3 min
@timed root = guess_root(detLMat, Re_k_min, Re_k_max, r_mesh, samp = 10^3)
# Exporting
...
# function find_Im_k_small(Re_k_min, Re_k_max, r_mesh, integrand_fun; max_iterations=3)
#     # Initial calculation of the integral
#     integral, error_int = quadgk(t -> integrand_fun(t, Re_k_min, Re_k_max, -r_mesh, 0.0), 0, 4, atol=int_root_atol)

#     # Initial estimate for im_min_est
#     if real(integral) >= 1
#         im_min_est = -r_mesh / real(integral)
#     else
#         im_min_est = -r_mesh
#     end
...
#     # Loop to adjust im_min_est based on integral value
#     while iteration < max_iterations
#         # Recalculate the integral with the updated im_min_est
#         integral, _ = quadgk(t -> integrand_fun(t, Re_k_min, Re_k_max, im_min_est, 0.0), 0, 4, atol=int_root_atol)
        
#         # If the real part of the integral is greater than or equal to 1, break the loop
#         if real(integral) >= 1
#             break
#         else
#             # Update im_min_est if the integral condition is not met
#             im_min_est = im_min_est - r_mesh / 10
#         end
...
#     # If the loop ends without meeting the condition, throw an error
#     if real(integral) < 0.5
#         error("Increase max_iterations to find roots")
#     end
...
#@timed integral, error_int = quadgk(t -> integrand_fun(t, Re_k_min, Re_k_max, -r_mesh, 0.0), 0, 4, atol=int_root_atol)
#@timed integral, error_int = quadgk(t -> integrand_fun(t, Re_k_min, Re_k_max, -r_mesh/4, 0.0), 0, 4, atol=int_root_atol)


# ########### SAVED DATA #######
# # ~15 min 
# @timed Im_k_small = find_Im_k_small(Re_k_min, Re_k_max, r_mesh, integrand_fun, max_iterations=3)
# @save joinpath(dirname(@__FILE__), "out_Im_k_small.jld2") Im_k_small
# @load joinpath(dirname(@__FILE__), "out_Im_k_small.jld2") Im_k_small
# #### END SAVED DATA ####

# Im_k_min = 30*Im_k_small
# -r_mesh*30
# Im_k_max = 0.0
...
# ###### SAVED DATA ####
# # 20 sec
# @timed (Re_k_range, Im_k_range) = redef_part(Re_k_min, Re_k_max, Im_k_min, Im_k_max, 1, 1, 5, 100; new_r_mesh=r_mesh)
# @save joinpath(dirname(@__FILE__), "out_Re_k_range.jld2") Re_k_range
# @save joinpath(dirname(@__FILE__), "out_Im_k_range.jld2") Im_k_range
# # @load joinpath(dirname(@__FILE__), "out_Re_k_range.jld2") Re_k_range
# # @load joinpath(dirname(@__FILE__), "out_Im_k_range.jld2") Im_k_range
# ######## END SAVED DATA #######

# Re_k_min = Re_k_range[1]
# Re_k_max = Re_k_range[2]
# Im_k_min = Im_k_range[1]
# Im_k_max = 0.0#Im_k_range[2]

# ######### SAVED DATA ###
# # numb of roots integral  - 36 sec
# @timed integral, error_int = quadgk(t -> integrand_fun(t, Re_k_min, Re_k_max, Im_k_min, Im_k_max), 0, 4, atol=int_root_atol/10)
# @save joinpath(dirname(@__FILE__), "out_integral_numb_roots.jld2") integral
# @save joinpath(dirname(@__FILE__), "out_error_int_numb_roots.jld2") error_int
# # @load joinpath(dirname(@__FILE__), "out_integral_numb_roots.jld2") integral
# # @load joinpath(dirname(@__FILE__), "out_error_int_numb_roots.jld2") error_int
# # refinement
# ###### END SAVED DATA ########


# if error_int > int_root_check_tol || abs(imag(integral)) > int_root_check_tol
#     Im_k_min = 1.1*Im_k_min
#     integral, error_int = quadgk(t -> integrand_fun(t, Re_k_min, Re_k_max, Im_k_min, Im_k_max), 0, 4, atol=int_root_atol)
# end
# # checking
# if error_int > int_root_check_tol || abs(imag(integral)) > int_root_check_tol
#     error("Large integral error in finding roots! Try to reshape the research region (safety_fact)!")
# end

# # # number of roots
# numb_roots = Int(round(integral))
# # # refined mesh
# # numb_re_int_s = Int(round(sqrt(numb_roots)))
# # numb_im_int_s = Int(round(sqrt(numb_roots)))
# # Delta_k_re_s = (Re_k_max - Re_k_min) / numb_re_int_s
# # Delta_k_im_s = (Im_k_max - Im_k_min) / numb_im_int_s
# # new_r_mesh_s = minimum([Delta_k_re_s, Delta_k_im_s, r_mesh])

# # Delta_k_re_s = (Re_k_max - Re_k_min) / numb_roots
# # Delta_k_im_s = (Im_k_max - Im_k_min) / numb_roots
# # new_r_mesh_s = minimum([Delta_k_re_s, Delta_k_im_s, r_mesh])


# open(joinpath(dirname(@__FILE__), "out_data.txt"), "a") do file
#     write(file, "\n")  # newline
#     write(file, "Region [re_min, re_max, im_min, im_max]: [$Re_k_min, $Re_k_max, $Im_k_min, $Im_k_max]")
#     write(file, "\n")
#     write(file, "Number of solutions: $numb_roots")
#     write(file, "\n")
...
#### Determing small regions with (ideally) single roots #####
...
# # Function to return the smallest eigenvalue of a complex matrix
# function smallest_eigenvalue(A)
#     # Compute all eigenvalues of matrix A
#     eigenvalues = eigvals(Matrix(A))
    
#     # Return the smallest eigenvalue by its absolute value
#     return minimum(abs.(eigenvalues))
# end

# # Function to find the minimum smallest eigenvalue in the region defined by re_k_min, re_k_max, im_k_min, im_k_max
# function find_minimum_eigenvalue_in_region(re_k_min::Float64, re_k_max::Float64, im_k_min::Float64, im_k_max::Float64, num_points::Int)
#     # Initialize the minimum eigenvalue and the corresponding k
#     min_eigenvalue = Inf + 0.0im
#     best_k = 0.0 + 0.0im

#     # Generate the grid of k values (real and imaginary parts)
#     re_range = range(re_k_min, re_k_max, length=num_points)  # Real part grid
#     im_range = range(im_k_min, im_k_max, length=num_points)  # Imaginary part grid

#     # Iterate over the grid of complex k values
#     for re_k in re_range
#         for im_k in im_range
#             k = Complex(re_k, im_k)  # Construct the complex k value

#             # Compute the matrix A(k) and find its smallest eigenvalue
#             A_k = LMat(k)
#             smallest_ev = smallest_eigenvalue(A_k)

#             # Update the minimum eigenvalue and corresponding k if a smaller one is found
#             if abs(smallest_ev) < abs(min_eigenvalue)
#                 min_eigenvalue = smallest_ev
#                 best_k = k
#             end
#         end
#     end

#     return min_eigenvalue, best_k
# end

# min_eigenvalue_arr = fill(0.0, numb_re_int, numb_im_int)
# @floop for (i, j) in product(1:numb_re_int,1:numb_im_int)
#     min_eigenvalue_arr[i,j], _ = find_minimum_eigenvalue_in_region(re_k_ext[i], re_k_ext[i+1], im_k_ext[j], im_k_ext[j+1], 35)
# end

# A_k = LMat(1.0)
# smallest_ev = smallest_eigenvalue(A_k)
# eigenvalues = eigvals(Matrix(A_k))
...
# function reg_with_few_sol(Re_k_min, Re_k_max, Im_k_min, Im_k_max,  numb_roots; max_iter=300, en_fac_mesh=100, ref_fac=1, new_r_mesh=r_mesh, redef=false, parall=true)
...
#     Delta_k_re = (Re_k_max - Re_k_min) / numb_re_int
#     Delta_k_im = (Im_k_max - Im_k_min) / numb_im_int
#     #new_r_mesh=minimum([Delta_k_re, Delta_k_im])/en_fac_mesh
...
#                 @warn "Something wrong with the integrals! Try to change ref_fac"
#             end
#         end
#     end

#     # consistency test
#     numb_sol_matrix = Int.(round.(integrals))
...
#     ind_numb_sol = [[] for i=1:max_numb_sol_matrix]
#     regions_with_roots = [[] for i=1:max_numb_sol_matrix]
#     for i = 1:max_numb_sol_matrix
...
#     return regions_with_roots
# end

# ProgressMeter@timed research_int_with_deg, n_recursion = recursive_refinement(regions_with_roots, 7)
...
# refined_regions_with_roots = [[] for _ in 1:length(regions_with_roots)]
# # Copy regions with a single solution (no refinement needed)
# refined_regions_with_roots[1] = regions_with_roots[1]
# i=2
# for j in eachindex(regions_with_roots[i])
# region = regions_with_roots[i][j]
# refined_subregions = reg_with_few_sol(region[1], region[2], region[3], region[4], i; max_iter=300, en_fac_mesh=100, ref_fac=ref_fact)
# refined_subregions[1]
# refined_subregions[2]
# refined_regions_with_roots[1]
# refined_regions_with_roots[2]
...
# non deb ~10 min
@timed research_int_with_deg, n_recursion = recursive_refinement(regions_with_roots, 7; debug=false)
@save joinpath(dirname(@__FILE__), "out_research_int_with_deg.jld2") research_int_with_deg
...
# generate a grid of points, shuffle and apply NR until reaching tolerance
# function newton_raphson_region(detLMat, region; max_iter_nr=100, grid_size=20, tolerance=1e-5, show_plot=true)
#     re_min, re_max, im_min, im_max = region
...
#     initial_points = shuffle!(vec([Complex(re, im) for re in re_vals, im in im_vals]))
...
#     point_iter = 0  # Track the number of grid iterations
#     nr_iter = 0  # Track the number of Newton-Raphson iterations
#     while point_iter < max_point_iter && !converged
#         point_iter = point_iter + 1
#         # Choose a random point in the region as the starting point
#         z0 = initial_points[point_iter]
...
#             push!(path, z)
#             nr_iter = i
#             if abs(fz) < tolerance && re_min <= real(z) <= re_max && im_min <= imag(z) <= im_max
#                 converged = true  # Set convergence status to true
#                 break  # Convergence when the value is sufficiently small
#             end
#         end

#         # Check if the final root is within the specified region and has converged
#         if converged
#             if show_plot
#                 # Create the heatmap of the region
...
#                 # Add the Newton-Raphson path
#                 path_re = real.(path)
#                 path_im = imag.(path)
#                 plot!(path_re, path_im, label="Path", marker=:circle, linecolor=:red, linewidth=2)
...
#     if !converged
#         return nothing, point_iter, nr_iter
#     end
...
# try1, _, _ = newton_raphson_region(detLMat, research_int[10]; max_iter_nr=100, grid_size=20, tolerance=1e-4, show_plot=true)
# abs(detLMat(try1))

# try2, _, _ = newton_raphson_region(detLMat, research_int[100]; max_iter_nr=100, grid_size=20, tolerance=1e-4, show_plot=true)
# try2, _, _ = newton_raphson_region(detLMat, research_int[100]; max_iter_nr=100, grid_size=20, tolerance=1e-3, show_plot=true)
# abs(detLMat(try2))
...
##### SAVED DATA ########
# 7 min
@timed roots_attempt_nr = roots_attempt_nr_func(research_int)
...
# # Define the ranges for the real and imaginary parts of k
# x_vals = range(9.0, stop=12.0, length=25)  # Grid for the real part of k
# y_vals = range(-0.05, stop=0.0, length=25)   # Grid for the imaginary part of k

# # Define the range for D0
# D0_vals = range(0.0, stop=0.05, length=25)  # Grid for D0

# # Calculate the total number of iterations
# total_iterations = length(x_vals) * length(y_vals) * length(D0_vals)
# max_k = 0.0 + 0.0im
# max_D0 = 0.0
# max_diff = 0.0 
# # Initialize progress bar
...
#             diff = maximum(abs.(LMatGain_num(k, D0) .- LMatGain_dense(k, D0)))
#             if diff > max_diff
...
# println("Maximum difference: $max_diff found at k = $max_k and D0 = $max_D0")
...
#         k = Complex(x, y)  # Construct the complex k = x + yi
#             # Calculate the absolute difference for the current (k, D0)
#             diff = maximum(abs.(LMatGain_num(k, 0.0) .- LMat(k)))
#             if diff > max_diff
#                 # Update the maximum difference and store the corresponding (k, D0)
#                 max_diff = diff
#                 max_k = k
...
## trying NR
@timed try1, _ = newton_raphson_gain(detLMatGain, kroots[1], 0, delta_D0, sm_eigvec[1], nij, fibersLength, NEI, NEO, f_values[1]; max_iter_nr=1000, tolerance=1e-9, show_plot=false, meth=2)
try1
...
# # Calculate the mean of the last two elements for each array in all_D0s_paths
# means_all_D0s_paths = mean_last_two_elements(all_D0s_paths)
# means_all_paths_th_0

# # Define the file path for saving
# save_file_prova = joinpath(@__DIR__, "data_prova.jld2")

# # Save the variables to the JLD2 file
# @save save_file_prova all_paths all_D0s_paths all_D0s
# @load save_file_prova all_paths all_D0s_paths all_D0s
...
#### TEST Matrix
#T_complex_num = Matrix{Complex{Float64}}(undef, length(kroots), length(kroots))

# part_int_ij = [0.0 + 0.0im for (mu, nu, i) in product(eachindex(kroots), eachindex(kroots), 1:NE)]
# int_lines_err_ij = [0.0 for (mu, nu, i) in product(eachindex(kroots), eachindex(kroots), 1:NE)]
# integrand_funct_T = Array{Function}(undef, length(kroots), length(kroots), NE)
# for (mu, nu, i) in product(eachindex(kroots), eachindex(kroots), 1:NE)
#     integrand_funct_T[mu, nu, i] = x -> gamma_perp^2/((k_th_0_and_D0[nu][1] - k_a)^2+gamma_perp^2) *
#         Psi_mu(x, k_th_0_and_D0[mu], n_sm_eigvec_gain[mu], fibersLength, NEI, NEO)[i]^2 *
#         abs(Psi_mu(x, k_th_0_and_D0[nu], n_sm_eigvec_gain[nu], fibersLength, NEI, NEO)[i])^2 *
#         delta_pump[i]
# end

# @timed begin # 2 h 20 m
#     @floop for (mu, nu, i) in product(eachindex(kroots), eachindex(kroots), 1:NE) 
#         part_int_ij[mu, nu, i], int_lines_err_ij[mu, nu, i] = quadgk(integrand_funct_T[mu, nu, i], 
#             0, fibersLength[i], atol=tol_int_lines) 
#     end
# end
# for (mu, nu, i) in product(eachindex(kroots), eachindex(kroots), 1:NE) 
#     if int_lines_err_ij[mu, nu, i] > tol_int_lines*10
#         @warn "Error in the T calculation for (μ, ν) = ($mu, $nu)!"
#     end
# end
# @floop for (mu, nu) in product(eachindex(kroots), eachindex(kroots))
#     T_complex_num[mu,nu] = sum(part_int_ij[mu, nu, i] for i in 1:NE)
# end

# @save joinpath(dirname(@__FILE__), "out_T_complex_num.jld2") T_complex_num
#@load joinpath(dirname(@__FILE__), "out_T_complex_num.jld2") T_complex_num
######### END SAVED DATA ###################
...
function T_mat_calc(k_th_0_and_D0, n_sm_eigvec_gain,  k_a, gamma_perp, fibersLength, delta_pump, NEI, NEO, NE)
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE)
        if k_modif[nu, i] - conj(k_modif[nu, i]) + 2*k_modif[mu, i] == 0
            AAux[mu, nu, i] = fibersLength[i]
        else
            AAux[mu, nu, i] = (exp(im*(k_modif[nu, i] - conj(k_modif[nu, i]) + 2*k_modif[mu, i])*
                fibersLength[i]) - 1)/
                (im*(k_modif[nu, i] - conj(k_modif[nu, i]) + 2*k_modif[mu, i]))
        end
    end

    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        if (k_modif[nu, i] - conj(k_modif[nu, i]) - 2*k_modif[mu, i]) == 0
            BAux[mu, nu, i] = fibersLength[i]
        else
            BAux[mu, nu, i] =  exp(2*im*k_modif[mu, i]*fibersLength[i]) * 
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        CAux[mu, nu, i] = (exp(im*(k_modif[nu, i] + 2*k_modif[mu, i])*fibersLength[i]) - 
            exp(-im*conj(k_modif[nu, i])*fibersLength[i]))/
            (im*(k_modif[nu, i] + conj(k_modif[nu, i]) + 2*k_modif[mu, i]))
    end

    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        DAux[mu, nu, i] = (exp(im*k_modif[nu, i]*fibersLength[i]) -
            exp(im*(2*k_modif[mu, i] - conj(k_modif[nu, i]))*fibersLength[i]))/
            (im*(k_modif[nu, i] + conj(k_modif[nu, i]) - 2*k_modif[mu, i]))
    end

    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        EAux[mu, nu, i] = exp(im*k_modif[mu, i]*fibersLength[i]) * 
        (exp(im*(k_modif[nu, i] - conj(k_modif[nu, i]))*fibersLength[i]) - 1)/
        (im*(k_modif[nu, i] - conj(k_modif[nu, i])))
    end

    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        FAux[mu, nu, i] = exp(im*k_modif[mu, i]*fibersLength[i]) * 
        (exp(im*k_modif[nu, i]*fibersLength[i]) - exp(-im*conj(k_modif[nu, i])*fibersLength[i]))/
        (im*(k_modif[nu, i] + conj(k_modif[nu, i])))
    end
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        ...
        IntMatr[mu, nu, i][3, 1] = DAux[mu, nu, i]
        IntMatr[mu, nu, i][3, 2] = FAux[mu, nu, i]
        IntMatr[mu, nu, i][3, 3] = FAux[mu, nu, i]
        IntMatr[mu, nu, i][3, 4] = CAux[mu, nu, i]
        IntMatr[mu, nu, i][4, 1] = BAux[mu, nu, i]
        IntMatr[mu, nu, i][4, 2] = EAux[mu, nu, i]
        IntMatr[mu, nu, i][4, 3] = EAux[mu, nu, i]
        IntMatr[mu, nu, i][4, 4] = AAux[mu, nu, i]
    end
...
function T_mat_calc_2(k_th_0_and_D0, n_sm_eigvec_gain,  k_a, gamma_perp, fibersLength, delta_pump, NEI, NEO, NE)
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE)
        numerator = -1 + exp(2im * fibersLength[i] * (k_modif[mu, i] + k_modif[nu, i] - 
            real(k_modif[nu, i])))
        denominator = 2im * k_modif[mu, i] - 2 * imag(k_modif[nu, i])
        if denominator == 0
            AAux[mu, nu, i] = fibersLength[i]
        else
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        numerator = -1im * (exp(2im * k_modif[mu, i] * fibersLength[i]) - exp(-2 * fibersLength[i] * imag(k_modif[nu, i])))
        denominator = 2 * k_modif[mu, i] - 2im * imag(k_modif[nu, i])
        if -k_modif[mu, i] + k_modif[nu, i] - real(k_modif[nu, i]) == 0
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE) 
        numerator = -1im * exp(-1im * fibersLength[i] * conj(k_modif[nu, i])) * (-1 + exp(2im * fibersLength[i] * (k_modif[mu, i] + real(k_modif[nu, i]))))
        denominator = 2 * (k_modif[mu, i] + real(k_modif[nu, i]))
        if denominator == 0
            CAux[mu, nu, i] = exp(-1im * fibersLength[i] * conj(k_modif[nu, i])) * fibersLength[i]
        else
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE)
        numerator = -1im * exp(-1im * fibersLength[i] * conj(k_modif[nu, i])) * (exp(2im * k_modif[mu, i] 
            * fibersLength[i]) - exp(2im * fibersLength[i] * real(k_modif[nu, i])))
        denominator = 2 * (k_modif[mu, i] - real(k_modif[nu, i]))
        if -2 * k_modif[mu, i]  + k_modif[nu, i]  +  conj(k_modif[nu, i]) == 0
            DAux[mu, nu, i] =  exp(1im * (2 * k_modif[mu, i] * fibersLength[i] - fibersLength[i] * 
                conj(k_modif[nu, i]))) * fibersLength[i]
        else
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE)
        numerator = exp(1im * k_modif[mu, i] * fibersLength[i] - 2 * fibersLength[i] * imag(k_modif[nu, i])) * (-1 + exp(2 * fibersLength[i] * imag(k_modif[nu, i])))
        denominator = 2 * imag(k_modif[nu, i])
        if denominator == 0
            EAux[mu, nu, i] = exp(1im * k_modif[mu, i] * fibersLength[i] - 2 * fibersLength[i] *
                imag(k_modif[nu, i])) * fibersLength[i]
        else
    ...
    @floop for (mu, nu, i) in product(1:N_roots, 1:N_roots, 1:NE)
        numerator = -1im * exp(1im * fibersLength[i] * (k_modif[mu, i] - conj(k_modif[nu, i]))) *
            (-1 + exp(2im * fibersLength[i] * real(k_modif[nu, i])))
        denominator = 2 * real(k_modif[nu, i])
        if denominator == 0
            FAux[mu, nu, i] = exp(1im * fibersLength[i] * 
                (k_modif[mu, i] - conj(k_modif[nu, i]))) * fibersLength[i]
        else 
...
#@timed T_complex_1 = T_mat_calc(k_th_0_and_D0, n_sm_eigvec_gain, k_a, gamma_perp, fibersLength, delta_pump, NEI, NEO, NE)

#@timed T_complex_2 = T_mat_calc_2(k_th_0_and_D0, n_sm_eigvec_gain, k_a, gamma_perp, fibersLength, delta_pump, NEI, NEO, NE)

@timed T_complex_3 = T_mat_calc_3(k_th_0_and_D0, n_sm_eigvec_gain, k_a, gamma_perp, fibersLength, delta_pump, NEI, NEO, NE)

# maximum(abs.(T_complex_1 .- T_complex_2) ./ (abs.(T_complex_2) .+ eps()))
# maximum(abs.(T_complex_1 .- T_complex_3) ./ (abs.(T_complex_1) .+ eps()))

#mean(abs.(T_complex_3 .- T_complex_num) ./ (abs.(T_complex_num) .+ eps()))
#maximum(abs.(T_complex_3 .- T_complex_num) ./ (abs.(T_complex_num) .+ eps()))

# Calculate the absolute error
#error_abs = abs.(T_complex_3 .- T_complex_num) ./ (abs.(T_complex_num) .+ eps())
# Create the heatmap
#heatmap(error_abs, title="Absolute Error Heatmap", xlabel="Column", ylabel="Row", color=:viridis)
# Find the indices where the absolute error exceeds 10^-3
#threshold = 1e-1
# indices_exceeding_threshold = findall(x -> x > threshold, error_abs)
# length(indices_exceeding_threshold)/length(T_complex_num)
# Extract the corresponding error values
#exceeding_errors = [error_abs[ind] for ind in indices_exceeding_threshold]
# Sort the errors in descending order and take the top 5
#top_errors = sort(exceeding_errors, rev=true)[1:5]

T_complex = T_complex_3
...
# T_complex[12,45]
# check_T_ratio = [0.0 for (_,_) in product(eachindex(kroots), eachindex(kroots))]
# @floop for (mu, nu) in product(eachindex(kroots), eachindex(kroots))
#     check_T_ratio[mu, nu] = abs.(imag.(T_complex))[mu, nu]/abs.(real.(T_complex))[mu, nu]
# end
# maximum(check_T_ratio)
# minimum(check_T_ratio)
# mean(check_T_ratio)
# heatmap(check_T_ratio, title="im/real Heatmap", xlabel="Column", ylabel="Row", color=:viridis)
# # Find the indices where the absolute error exceeds 10^-3
# threshold = 5*1e-1
# indices_exceeding_threshold_ratio = findall(x -> x > threshold, check_T_ratio)
# length(indices_exceeding_threshold_ratio)/length(T_complex)
# # Extract the corresponding error values
# exceeding_ratio = [check_T_ratio[ind] for ind in indices_exceeding_threshold_ratio]
# # Sort the errors in descending order and take the top 5
# top_ratio = sort(exceeding_ratio, rev=true)[1:5]
# ### Check NaN 
# if any(isnan.(real(T_complex)) .| isnan.(imag(T_complex)))
#     @warn "Warning: The matrix contains NaN values."
# end


##### real matrx approximation
T_real_complete = real.(T_complex)
# minimum(T_real_complete)
# maximum(T_real_complete)
# # Indices negative elements
# negative_indices = [(i, j) for i in 1:size(T_real_complete, 1), j in 1:size(T_real_complete, 2) if T_real_complete[i, j] < 0]
# length(negative_indices)/(size(T_real_complete, 1)*size(T_real_complete, 2))
# heatmap(T_real_complete, title="T real matrx", xlabel="Column", ylabel="Row", color=:viridis)
# T_real_negative = map(x -> x > 0 ? 0 : x, T_real_complete)
# heatmap(T_real_negative, title="T negative matrx", xlabel="Column", ylabel="Row", color=:viridis)
# # Extract negative elements from T_real using the negative_indices
# negative_elements_T = [T_real_complete[i, j] for (i, j) in negative_indices]
# sort(negative_elements_T)[1:3]


# # Create a new matrix where negative elements are set to zero
# T_real_without_neg = map(x -> x < 0 ? 0 : x, T_real_complete)
# heatmap(T_real_without_neg, title="T real matrx", xlabel="Column", ylabel="Row", color=:viridis)

T_real = T_real_complete


# # List to store ratios that exceed 1
# exceeding_ratios_column = []

# # Loop through the matrix elements
# for i in 1:size(T_real, 1)  # Iterate over the rows
#     for j in 1:size(T_real, 2)  # Iterate over the columns
#         # Calculate the ratio only if i ≠ j (we avoid the diagonal ratio T_real[i,i] / T_real[i,i] which is 1)
#         if i != j
#             ratio = T_real[i, i] / T_real[i, j]
#             if ratio < 1
#                 # Store the indices and the value of the ratio
#                 push!(exceeding_ratios_column, (i, j, ratio))
#             end
#         end
#     end
# end

# length(exceeding_ratios_column)/length(T_real[1,:])/length(T_real[:,1])


# # List to store ratios that exceed 1
# exceeding_ratios_row = []


# # Loop through the matrix elements
# for i in 1:size(T_real, 1)  # Iterate over the rows for the diagonal element T_real[i, i]
#     for j in 1:size(T_real, 1)  # Iterate over the rows for the non-diagonal element T_real[j, i]
#         # Calculate the ratio only if i ≠ j to avoid division of the diagonal element by itself
#         if i != j
#             ratio = T_real[i, i] / T_real[j, i]
#             if ratio < 1
#                 # Store the indices and the value of the ratio
#                 push!(exceeding_ratios_row, (i, j, ratio))
#             end
#         end
#     end
# end
...
# length(exceeding_ratios_row)/length(T_real[1,:])/length(T_real[:,1])




# function T_overlap_calc(k_th_0_and_D0, n_sm_eigvec_gain,  k_a, gamma_perp, fibersLength, delta_pump, NEI, NEO, NE)
...
#     T_complex = Matrix{Complex{Float64}}(undef, N_roots, N_roots)
#     @floop for (mu, nu) in product(1:N_roots, 1:N_roots)
#         T_complex[mu,nu] = sum(delta_pump[i] * TAux[mu, nu, i] for i in 1:NE)
#     end

#     return T_complex
# end

# T_overlap = T_overlap_calc(k_th_0_and_D0, n_sm_eigvec_gain, k_a, gamma_perp, fibersLength, delta_pump, NEI, NEO, NE)
# heatmap(abs.(T_overlap), title="T real matrx", xlabel="Column", ylabel="Row", color=:viridis)
# T_overlap[10,10]
# T_overlap[10,16]

# ##### sequence of lasing modes
# # the first D_{0,int}^\mu is the smallest D_0^\mu
# ordered_modes_first = [argmin(all_D0s_th_0)]
# D_0_int_first = [all_D0s_th_0[ordered_modes_first[1]]]
...
#     # First pass to find the global min and max intensities
#     for (i, _) in enumerate(fibersPoints)
#         xs = range(0, stop=fibersLength[i], length=100)
#         for x in xs
#             fiber_intensity = fibers_intens(x, I_0, k_th_0_and_D0, n_sm_eigvec_gain, ordered_modes, fibersLength, NEI, NEO)[i]
...
#     # Second pass to plot the fibers with segments
#     for (i, points) in enumerate(fibersPoints)
#         # Calculate the number of points proportionally to the fiber length
#         num_samples = maximum([2, Int(round(base_num_points * (fibersLength[i] / max_fiber_length)))])
...
#         # Generate the points along the fiber
#         x_vals = [(1 - t) * x_start + t * x_end for t in xs / fibersLength[i]]
#         y_vals = [(1 - t) * y_start + t * y_end for t in xs / fibersLength[i]]
...
#         # Normalize the intensities for coloring
#         norm_intensities = [(intensity - min_intensity) / (max_intensity - min_intensity) for intensity in intensities]

#         # Draw small segments between consecutive points
#         for j in 1:(num_samples - 1)
#             plot!([x_vals[j], x_vals[j + 1]], [y_vals[j], y_vals[j + 1]], color=:inferno, zcolor=norm_intensities[j], linewidth=1, label="")
#         end
#     end

#     # Finalize the plot with equal aspect ratio and automatic colorbar
#     plot!(legend=false, aspect_ratio=:equal, colorbar_title="Intensity", clim=(min_intensity, max_intensity))
# end
